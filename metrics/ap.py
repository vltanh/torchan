...
def get_avg_precision_at_iou(gt_boxes, pred_bb, iou_thr=0.5):
    ...
    for ithr, model_score_thr in enumerate(sorted_model_scores[:-1]):
        ...
        img_ids = \
            gt_boxes.keys() if ithr == 0 else model_scores[model_score_thr]
        for img_id in img_ids:
            ...
            # print(img_id)
            img_results[img_id] = \
                get_single_image_results(gt_boxes_img,
...
class AP():
    ...
    def calculate(self, output, target):
        image_ids = [x['image_id'].item() for x in target]
        pred_bboxes = {image_id: {'boxes': x['boxes'][:len(x['scores'])].detach().cpu(),
                                  'scores': x['scores'][:].detach().cpu()}
                       for image_id, x in zip(image_ids, output)}
        gt_bboxes = {image_id: x['boxes'].detach().cpu()
                     for image_id, x in zip(image_ids, target)}
        return pred_bboxes, gt_bboxes

    def update(self, value):
        pred_bboxes, gt_bboxes = value
        self.pred_bboxes.update(pred_bboxes)
        self.gt_bboxes.update(gt_bboxes)

    def reset(self):
        self.pred_bboxes = dict()
        self.gt_bboxes = dict()
        self.result = dict()

    def value(self):
        self.result = get_avg_precision_at_iou(self.gt_bboxes,
                                               self.pred_bboxes,
                                               self.iou_thr)
        return self.result['avg_prec']

    def summary(self):
        print('Average Precision:', self.result['avg_prec'])
